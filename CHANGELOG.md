...
- src/features/customers/hooks/customers/customerPortalQueries.ts: constantes de columnas, `pageBounds`, `parseRpcPage`, `fetchForkliftsBriefMap` y los diez hooks; `usePortalScope` unifica `useAuth` + `useVerifiedIdentityScope` sin alterar claves ni activación.
