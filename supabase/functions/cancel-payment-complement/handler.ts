...
export async function handleCancelPaymentComplement(req: Request, deps: CancelRepDeps): Promise<Response> {
  ...
  try {
    ...
    const { payment_id, motive } = body as { payment_id?: unknown; motive?: unknown };
    if (!isUUID(payment_id)) {
      return json({ error: "payment_id must be a valid UUID" }, 400, jsonHeaders);
    }
    const motiveCode = typeof motive === "string" && VALID_MOTIVES.has(motive) ? motive : "02";

    const { data: payment } = await supabase
      .from("payments").select("rep_facturapi_id, rep_cfdi_status").eq("id", payment_id).single();
    if (!payment) {
    ...
    if (!cancelRes.ok) {
      ...
      return json({ error: `Facturapi cancel error: ${cancelRes.status}`, detail: errBody }, 502, jsonHeaders);
    }

    await supabase.from("payments")
      .update({ rep_cfdi_status: "cancelled", rep_cancelled_at: new Date().toISOString() })
      .eq("id", payment_id);
