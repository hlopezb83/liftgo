...
import { handleCancelPaymentComplement, type CancelRepDeps } from "./handler.ts";
import { buildSupabaseMock, type MockConfig } from "../_shared/test/supabaseClientMock.ts";
import { installFacturapiMock, facturapiBadRequest } from "../_shared/test/facturapiMock.ts";
...
function makeRequest(body: unknown, opts: { auth?: string | null } = {}): Request {
  ...
  if (opts.auth !== null) headers["Authorization"] = opts.auth ?? "Bearer test-token";
...
}): { deps: CancelRepDeps; serviceState: ReturnType<typeof buildSupabaseMock> } {
...
Deno.test("handler: 401 sin Authorization", async () => {
  const { deps } = makeDeps({});
  const res = await handleCancelPaymentComplement(makeRequest({ payment_id: PAYMENT_ID }, { auth: null }), deps);
...
Deno.test("handler: 403 si no admin/administrativo", async () => {
  ...
  const res = await handleCancelPaymentComplement(makeRequest({ payment_id: PAYMENT_ID }), deps);
...
Deno.test("handler: 400 si payment_id no es UUID", async () => {
  ...
  const res = await handleCancelPaymentComplement(makeRequest({ payment_id: "bad" }), deps);
...
Deno.test("handler: 400 si REP no está timbrado", async () => {
  const { deps } = makeDeps({
    service: {
      selects: {
        user_roles: { data: [{ role: "admin" }], error: null },
        payments: { data: { rep_cfdi_status: "draft", rep_facturapi_id: null }, error: null },
      },
    },
  });
  const res = await handleCancelPaymentComplement(makeRequest({ payment_id: PAYMENT_ID }), deps);
...
Deno.test("handler: 400 si no hay API key configurada", async () => {
  const { deps } = makeDeps({
    ...
    service: {
      selects: {
        user_roles: { data: [{ role: "admin" }], error: null },
        payments: { data: { rep_cfdi_status: "stamped", rep_facturapi_id: "fapi_xx" }, error: null },
...
Deno.test("handler: happy path llama Facturapi DELETE y marca cancelled", async () => {
  const mock = installFacturapiMock({
    "/invoices/fapi_xx": (req) => req.method === "DELETE"
  ...
  try {
    ...
    const res = await handleCancelPaymentComplement(makeRequest({ payment_id: PAYMENT_ID, motive: "02" }), deps);
    const body = await res.json();
    assertEquals(res.status, 200);
    assertEquals(body.success, true);

    const upd = serviceState.updates.find((u) => u.table === "payments" && u.patch.rep_cfdi_status === "cancelled");
    assert(upd, "expected cancelled update");
    assert(typeof upd!.patch.rep_cancelled_at === "string");
    assert(mock.calls.some((c) => c.method === "DELETE" && c.url.includes("motive=02")));
  } finally {
    mock.restore();
  }
});
...
Deno.test("handler: 502 si Facturapi falla y NO marca cancelled", async () => {
  ...
  try {
    ...
    assert(!serviceState.updates.some((u) => u.patch.rep_cfdi_status === "cancelled"));
  } finally {
    mock.restore();
  }
});
...
Deno.test("handler: motivo inválido cae a default '02'", async () => {
  ...
  try {
    ...
    const res = await handleCancelPaymentComplement(makeRequest({ payment_id: PAYMENT_ID, motive: "99" }), deps);
    await res.json();
    assertEquals(res.status, 200);
