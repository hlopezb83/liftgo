...
import { isValidRegimenFiscalCode, resolveReceptorRegimenFiscal } from "./regimenFiscal.ts";

Deno.test("receptor global: etiqueta heredada se normaliza a 616 (NC)", () => {
  assertEquals(resolveReceptorRegimenFiscal(true, "616 - Sin obligaciones fiscales"), "616");
});
...
Deno.test("receptor no global: se conserva el valor recortado y la validación", () => {
  ...
  assertEquals(isValidRegimenFiscalCode(resolveReceptorRegimenFiscal(false, " 601 ")), true);
