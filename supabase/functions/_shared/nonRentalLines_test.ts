...
Deno.test("FIX-6: descarta renta y venta, conserva extras", () => {
  const lines = extractNonRentalLines([
    {
      description: "Montacargas A — Renta mensual (ene)",
      quantity: 1,
      unit_price: 100,
      total: 100,
    },
    {
      description: "Venta de equipo XYZ",
      quantity: 1,
      unit_price: 500,
      total: 500,
    },
...
Deno.test("FIX-6: entradas inválidas o vacías devuelven []", () => {
  ...
  assertEquals(
    extractNonRentalLines([{ description: "Sin monto", total: 0 }]),
    [],
  );
});
