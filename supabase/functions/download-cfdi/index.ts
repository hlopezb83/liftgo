...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const allowed = (roles ?? []).some(
      (r) => r.role === "admin" || r.role === "administrativo" || r.role === "ventas",
    );
    if (!allowed) {
    ...
    const { data: invoice, error: invErr } = await supabase
      .from("invoices")
      .select("id, invoice_number, cfdi_uuid, cfdi_status, cfdi_xml, cfdi_xml_url, cfdi_pdf_url, facturapi_invoice_id")
      .eq("id", invoice_id)
      .single();
    if (invErr || !invoice) {
    ...
    // 1) Try Storage
    if (storagePath) {
      const { data: file } = await supabase.storage.from(BUCKET).download(storagePath);
      if (file) {
        return new Response(file, {
          headers: {
            ...corsHeaders,
    ...
    if (!invoice.facturapi_invoice_id) {
      return new Response(JSON.stringify({ error: "Missing facturapi reference" }), {
    ...
    const apiKey = mode === "live"
      ...
      : ((secrets?.facturapi_test_key as string | null) || Deno.env.get("FACTURAPI_TEST_KEY"));
    if (!apiKey) {
      return new Response(JSON.stringify({ error: "Facturapi key not configured" }), {
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({ error: `Facturapi error: ${res.status}`, detail: errBody.slice(0, 500) }), {
