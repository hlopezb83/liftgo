// Pure handler for download-cfdi, deps-injected for testability.
// SIN cambio de comportamiento observable respecto al Deno.serve original:
// misma autenticación (requireRole equivalente), mismas respuestas, mismos
// headers. Multiempresa · Fase 1: la organización se deriva del documento en
// BD (nunca del payload) antes de leer secretos o llamar al PAC.
...
// download-cfdi necesita `storage.download` además de `upload` (servir el
// archivo ya persistido en Storage sin re-descargarlo del PAC).
export interface DownloadSupabaseLike extends Omit<SupabaseLike, "storage"> {
  storage: {
    from: (bucket: string) => {
      upload: (
        path: string,
...
export interface DownloadCfdiDeps {
  createCallerClient: () => SupabaseLike;
  createServiceClient: () => DownloadSupabaseLike;
  fetchImpl: typeof fetch;
  env: (key: string) => string | undefined;
}

interface DownloadAuthOk {
  ok: true;
  userId: string;
  role: string;
  supabase: DownloadSupabaseLike;
}
interface DownloadAuthFail {
  ok: false;
  response: Response;
}
...
// de perfil, 403 cuenta desactivada / rol insuficiente).
async function authenticateDownload(
  req: Request,
  deps: DownloadCfdiDeps,
): Promise<DownloadAuthOk | DownloadAuthFail> {
  ...
  const roleRes = await supabase.from("user_roles").select("role").eq(
    "user_id",
    userId,
  ).in("role", ALLOWED_ROLES).maybeSingle();
  ...
  return { ok: true, userId, role: roleData.role, supabase };
}

async function enforceRateLimitWithDeps(
  req: Request,
  supabase: DownloadSupabaseLike,
  bucket: string,
  identifier: string,
...
export async function handleDownloadCfdi(
  ...
  try {
    const auth = await authenticateDownload(req, deps);
    if (!auth.ok) return auth.response;
    const supabase = auth.supabase;

    // N-44: rate limit de descargas (30 req/min por usuario; fail-closed).
    const limited = await enforceRateLimitWithDeps(
      req,
      supabase,
      "download-cfdi",
      auth.userId,
