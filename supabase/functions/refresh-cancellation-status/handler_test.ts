  ...
  const { deps } = makeDeps({
    service: {
      selects: {
        ...
        invoices: {
          data: { facturapi_invoice_id: null, organization_id: ORG_ID },
          error: null,
        },
      },
    },
  ...
  const { deps } = makeDeps({
    ...
    service: {
      selects: {
        user_roles: { data: [{ role: "admin" }], error: null },
        invoices: {
          data: { facturapi_invoice_id: "fapi_1", organization_id: ORG_ID },
          error: null,
        },
        company_settings: {
    ...
    const { deps, serviceState } = makeDeps({
      ...
      service: {
        selects: {
          user_roles: { data: [{ role: "admin" }], error: null },
          invoices: {
            data: { facturapi_invoice_id: "fapi_p", organization_id: ORG_ID },
            error: null,
          },
          company_settings: {
            data: { facturapi_mode: "test", organization_id: ORG_ID },
    ...
    const { deps } = makeDeps({
      ...
      service: {
        selects: {
          user_roles: { data: [{ role: "admin" }], error: null },
          invoices: {
            data: { facturapi_invoice_id: "fapi_e", organization_id: ORG_ID },
            error: null,
          },
          company_settings: {
            data: { facturapi_mode: "test", organization_id: ORG_ID },
...
Deno.test("refresh-cancellation: MULTIEMPRESA sin credenciales propias en modo live rechaza (no reutiliza llaves ajenas)", async () => {
  ...
  assertEquals(
    ...
    0,
    "no debe actualizarse sin key propia",
  );
});
