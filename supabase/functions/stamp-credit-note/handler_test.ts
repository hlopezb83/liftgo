...
}): { deps: StampCreditNoteDeps; serviceState: ReturnType<typeof buildSupabaseMock> } {
...
Deno.test("handler: rejects without Authorization header (401)", async () => {
  const { deps } = makeDeps({});
  const res = await handleStampCreditNote(makeRequest({ credit_note_id: NC_ID }, { auth: null }), deps);
...
Deno.test("handler: returns 403 when user is not admin/administrativo", async () => {
  ...
  const res = await handleStampCreditNote(makeRequest({ credit_note_id: NC_ID }), deps);
...
Deno.test("handler: returns 400 when credit_note_id is not a UUID", async () => {
  ...
  const res = await handleStampCreditNote(makeRequest({ credit_note_id: "bad" }), deps);
...
Deno.test("handler: returns 409 when credit note already stamped", async () => {
  const { deps } = makeDeps({
    service: {
      selects: {
        user_roles: { data: [{ role: "admin" }], error: null },
        credit_notes: { data: { id: NC_ID, cfdi_status: "stamped" }, error: null },
      },
    },
  });
  const res = await handleStampCreditNote(makeRequest({ credit_note_id: NC_ID }), deps);
...
Deno.test("handler: returns 400 when source invoice is not stamped", async () => {
  const { deps } = makeDeps({
    service: {
      selects: {
        user_roles: { data: [{ role: "admin" }], error: null },
        credit_notes: { data: { id: NC_ID, invoice_id: INVOICE_ID }, error: null },
        invoices: { data: { id: INVOICE_ID, cfdi_status: "draft" }, error: null },
      },
    },
  });
  const res = await handleStampCreditNote(makeRequest({ credit_note_id: NC_ID }), deps);
    ...
    const { deps, serviceState } = makeDeps({
      ...
      service: {
        selects: {
          user_roles: { data: [{ role: "admin" }], error: null },
          credit_notes: {
            data: { id: NC_ID, invoice_id: INVOICE_ID, tax_rate: 16, currency: "MXN", line_items: [{ description: "NC", quantity: 1, unit_price: 100 }] },
            error: null,
          },
  ...
  try {
    const { deps, serviceState } = makeDeps({
      ...
      service: {
        selects: {
          user_roles: { data: [{ role: "admin" }], error: null },
          credit_notes: { data: { id: NC_ID, invoice_id: INVOICE_ID, line_items: [] }, error: null },
          invoices: { data: STAMPED_INVOICE, error: null },
          company_settings: { data: { facturapi_mode: "test" }, error: null },
          billing_secrets: { data: null, error: null },
        },
