// Pure handler for cancel-credit-note, deps-injected for testability.
// Multiempresa · Fase 1: la organización se valida contra la NC leída de BD
...
export type CancelCreditNoteDeps = StampCfdiDeps;
...
export async function handleCancelCreditNote(
  ...
  try {
    // EC-A1: authenticateWithDeps = requireRole + bypass service_role JWT
    // para el consumer de cfdi_retry_queue (mismo patrón que antes tenía
    ...
    const { data: nc, error: ncErr } = await supabase
      .from("credit_notes")
      .select(
        "organization_id, cfdi_status, cancellation_status, facturapi_invoice_id",
      )
    ...
    const orgCheck = await resolveDocumentOrganization({
      ...
      documentOrganizationId: (nc as Record<string, unknown>)
        .organization_id as string | null | undefined,
    });
    if (!orgCheck.ok) {
      console.error("[cancel-credit-note] organization check failed", {
        credit_note_id,
    ...
    if (isStub && mode === "live") {
      ...
      return jsonError(
        ...
        400,
        !apiKey
          ? "Facturapi API key no configurada para esta empresa en modo live. No se puede cancelar sin llamar al SAT."
