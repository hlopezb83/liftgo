ALTER TABLE public.maintenance_logs
  ...
  ADD COLUMN IF NOT EXISTS e2e_scope text;

CREATE INDEX IF NOT EXISTS idx_maintenance_logs_is_e2e
  ON public.maintenance_logs (e2e_scope) WHERE is_e2e;

DROP TRIGGER IF EXISTS trg_guard_is_e2e ON public.maintenance_logs;
CREATE TRIGGER trg_guard_is_e2e
  BEFORE INSERT OR UPDATE ON public.maintenance_logs
  FOR EACH ROW EXECUTE FUNCTION public.guard_is_e2e_flag();
...
BEGIN
  ...
  VALUES (v_forklift_id, 'preventive', 'E2E Kanban WO - ' || substr(p_scope, 1, 16),
    0, CURRENT_DATE, 'pending', true, p_scope)
...
BEGIN
  ...
  DELETE FROM public.maintenance_parts
    WHERE maintenance_log_id IN (
      SELECT id FROM public.maintenance_logs
       WHERE is_e2e = true OR forklift_id IN (SELECT id FROM public.forklifts WHERE is_e2e = true));
  DELETE FROM public.maintenance_labor
    WHERE maintenance_log_id IN (
      SELECT id FROM public.maintenance_logs
       WHERE is_e2e = true OR forklift_id IN (SELECT id FROM public.forklifts WHERE is_e2e = true));
