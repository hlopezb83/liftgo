-- Fix 8.5 (Sprint 8): exceso de horas del contrato visible al cerrar devolución.
ALTER TABLE public.return_inspections
  ADD COLUMN IF NOT EXISTS extra_hours numeric,
  ADD COLUMN IF NOT EXISTS suggested_extra_hour_charge numeric;

COMMENT ON COLUMN public.return_inspections.extra_hours IS
  'Fix 8.5: horas por encima de max_hours_per_month * meses rentados del contrato.';
COMMENT ON COLUMN public.return_inspections.suggested_extra_hour_charge IS
  'Fix 8.5: cargo sugerido (extra_hours * contracts.extra_hour_rate). Informativo: NO se factura automáticamente.';
