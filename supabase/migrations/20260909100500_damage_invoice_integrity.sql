-- Integridad de daños/facturas:
--   * un daño sólo se factura después de reparar;
--   * crear factura + ligar daño es una sola transacción;
--   * un daño archivado no puede mutarse/facturarse;
--   * archivar conserva fotos y documentos para una futura restauración.

-- Overload transaccional. La firma existente de cuatro argumentos se conserva
-- para facturas ordinarias; PostgREST selecciona ésta cuando recibe damage_id.
...
BEGIN
  ...
  -- La función canónica crea factura + pivote. Al estar dentro de este RPC,
  -- cualquier error posterior revierte también esa creación.
...
BEGIN
  ...
  -- Una transición explícita a repaired sella el instante de reparación. En
  -- cualquier otro caso, estado físico y timestamp deben contar la misma verdad.
  IF TG_OP = 'UPDATE'
  ...
  -- La combinación facturada debe ser válida aun si la mutación sólo toca
  -- repaired_at/deleted_at. Así no se puede degradar silenciosamente una fila
  -- ya facturada a "sin reparar" dejando status/invoice_id intactos.
  ...
  -- La RPC de archivo aplica esta regla, pero el trigger la hace ineludible
  -- para UPDATE directos y service_role. Un daño ya reparado/facturado sí
  -- puede archivarse conservando su factura y evidencia.
  IF TG_OP = 'UPDATE'
...
BEGIN
  ...
  -- Un daño archivado es evidencia inmutable. Una eliminación física de la
  -- factura dejaría una referencia imposible de conservar, así que se rechaza;
  -- cancelar la factura sí es válido y la fila archivada se concilia al restaurar.
...
BEGIN
  ...
  -- Serializa el cierre de los daños de una misma unidad. Sin este lock, dos
  -- reparaciones simultáneas podían ver al otro daño aún abierto, omitir ambas
  -- la restauración y dejar el montacargas en maintenance para siempre.
  PERFORM 1
...
-- La reparación de filas históricas facturadas puede cambiar únicamente
-- `repaired_at`; el trigger debe escuchar ese cierre además del estado/archivo.
...
-- Archivar ya no destruye documentos y sólo libera equipos reparados.
...
BEGIN
  ...
  -- Los documentos se conservan; el padre archivado los oculta y restaurarlo
  -- recupera exactamente la misma evidencia.
  UPDATE public.damage_records
     SET deleted_at = now(), deleted_by = (select auth.uid()), updated_at = now()
   WHERE id = p_damage_id;

  IF v_rec.forklift_id IS NOT NULL THEN
    -- Mismo lock canónico del trigger de reparación: el último cierre siempre
    -- vuelve a evaluar el conjunto después del commit concurrente anterior.
    PERFORM 1
    FROM public.forklifts
...
-- a mantenimiento dentro de la misma transacción.
