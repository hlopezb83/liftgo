-- === CANONICAL === v7.209.0 (DIFF A1 + A2)
-- A1: get_income_statement multi-moneda. Multiplica subtotales por tipo_cambio
--     de cada documento (invoices y credit_notes) para reportar en MXN.
--     credit_notes no tiene columna tipo_cambio, hereda del invoice padre.
-- A2: get_portal_invoices ahora resta solo NCs con cfdi_status='stamped'
--     (borradores/errores ya no descuentan saldo del portal).

-- ============================================================
-- A1: get_income_statement (multi-currency → MXN)
-- ============================================================
...
BEGIN
  ...
  inv AS (
    SELECT
      i.id,
      -- v7.209.0 A1: normalizar a MXN multiplicando por tipo_cambio del doc.
      (i.subtotal * COALESCE(NULLIF(i.tipo_cambio, 0), 1))::numeric AS subtotal,
      i.invoice_type,
  ...
  cn_base AS (
    SELECT cn.id,
      -- credit_notes no tiene tipo_cambio: heredamos del invoice padre.
      (cn.subtotal * COALESCE(NULLIF(pi.tipo_cambio, 0), 1))::numeric AS subtotal,
...
-- ============================================================
-- A2: get_portal_invoices (NCs solo timbradas)
-- ============================================================
...
AS $function$
  ...
  LEFT JOIN (
    ...
    -- v7.209.0 A2: solo NCs timbradas descuentan saldo del portal.
