  ...
  SELECT json_build_object(
    ...
    'weekly_utilization', (
      ...
      FROM (
        SELECT
          ws.week_start,
          'Sem ' || EXTRACT(WEEK FROM ws.week_start)::int AS week_label,
          CASE WHEN active_count = 0 THEN 0
          ELSE LEAST(ROUND(
            (SELECT COUNT(DISTINCT (f2.id, d.d))
             FROM forklifts f2
             CROSS JOIN generate_series(ws.week_start, ws.week_start + 6, '1 day'::interval) d(d)
             WHERE f2.status NOT IN ('sold', 'retired')
               AND EXISTS (
                 SELECT 1 FROM bookings b
                 WHERE b.forklift_id = f2.id
                   AND b.status IN ('confirmed', 'completed')
                   AND b.start_date <= d.d::date
                   AND b.end_date >= d.d::date
               )
            )::numeric / (active_count * 7) * 100
          ), 100) END AS util_pct
        FROM (
          SELECT
            date_trunc('week', CURRENT_DATE)::date - (n * 7) AS week_start
