-- Integridad de importaciones bancarias, consultas paginadas y detalle del portal.

-- El fingerprint anterior truncaba la descripcion a 80 caracteres. Dos
-- movimientos distintos que compartian ese prefijo se trataban como el mismo.
DROP INDEX IF EXISTS public.bank_statement_lines_account_hash_occ_uq;
...
WITH recomputed AS (
    ...
    left(
      encode(
        extensions.digest(
          concat_ws(
            '|',
            to_char(l.posted_date, 'YYYY-MM-DD'),
            trim(to_char(l.signed_amount, 'FM9999999999990.00')),
  ...
  SELECT
    ...
    r.new_hash,
    row_number() OVER (
      PARTITION BY r.bank_account_id, r.new_hash
      ORDER BY r.id
    )::integer AS new_occurrence
  FROM recomputed r
)
UPDATE public.bank_statement_lines l
SET hash = n.new_hash,
    occurrence = n.new_occurrence
FROM numbered n
WHERE n.id = l.id;

CREATE UNIQUE INDEX bank_statement_lines_account_hash_occ_uq
  ON public.bank_statement_lines (bank_account_id, hash, occurrence);

COMMENT ON COLUMN public.bank_statement_lines.hash IS
  'SHA-256 truncado a 80 bits de fecha|monto|referencia|descripcion completa.';

-- La carga viaja en bloques pequenos a una zona de staging. Solamente finalize
-- escribe las tablas canonicas; asi un fallo de red o matching no deja una
-- importacion parcial y un retry puede recuperar el mismo resultado.
...
CREATE TABLE public.bank_statement_upload_chunks (
...
CREATE INDEX bank_statement_uploads_cleanup_idx
...
-- Staging no tiene acceso directo desde el navegador. Las RPC SECURITY DEFINER
-- verifican rol y propiedad en cada paso.
...
-- service_role bypasea RLS, pero las policies explícitas documentan que no hay
-- una ruta directa para anon/authenticated y satisfacen el guard de migraciones.
...
-- Los headers finalizados se conservan siete dias para que un retry posterior
-- a una respuesta perdida recupere el mismo resultado. Staging abandonado se
-- purga despues de 24 horas.
...
BEGIN
  ...
  FROM cron.job
  WHERE jobname = 'cleanup-bank-statement-uploads-hourly';
  ...
  PERFORM cron.schedule(
    'cleanup-bank-statement-uploads-hourly',
    '17 * * * *',
    $command$SELECT public.cleanup_bank_statement_uploads();$command$
  );
END;
$schedule$;

-- La via canonica ya no admite INSERT directo. Esto tambien fuerza a clientes
-- antiguos a fallar de forma segura en vez de omitir locks/fingerprints.
REVOKE INSERT ON TABLE public.bank_statement_imports FROM authenticated;
REVOKE INSERT ON TABLE public.bank_statement_lines FROM authenticated;

-- Retira cualquier version experimental de la RPC monolitica.
DROP FUNCTION IF EXISTS public.import_bank_statement_transactional(uuid, text, date, date, jsonb);

-- Pagina filtrada en servidor. El total_count corresponde al filtro completo,
-- no solamente a la pagina visible.
  ...
  WITH filtered AS (
    ...
    WHERE l.bank_account_id = p_bank_account_id
      ...
      AND (
        ...
        OR coalesce(l.reference, '') ILIKE '%' || btrim(p_search) || '%'
        OR abs(l.signed_amount)::text ILIKE '%' || btrim(p_search) || '%'
      )
  ), page_rows AS (
    SELECT *
      FROM filtered
...
-- cientos de reportes cerrados. SECURITY INVOKER reutiliza las RLS vigentes.
