...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'weekly_utilization', (
      WITH weeks AS (
        SELECT
          (DATE_TRUNC('week', CURRENT_DATE)::date - (w * 7))::date AS week_start,
          (DATE_TRUNC('week', CURRENT_DATE)::date - (w * 7) + 6)::date AS week_end
      ...
      SELECT COALESCE(json_agg(json_build_object(
        'week_label', 'S' || TO_CHAR(wk.week_start, 'IW'),
        'utilization', CASE
          WHEN fleet.total = 0 THEN 0
          ELSE ROUND(
            COALESCE((
              SELECT SUM(LEAST(b.end_date, wk.week_end) - GREATEST(b.start_date, wk.week_start) + 1)
              FROM bookings b
              JOIN forklifts f ON f.id = b.forklift_id
              WHERE b.status = 'confirmed'
                AND f.status NOT IN ('retired', 'sold')
                AND b.start_date <= wk.week_end
                AND b.end_date >= wk.week_start
            ), 0) * 100.0 / (fleet.total * 7.0)
          )
        END
      ) ORDER BY wk.week_start), '[]'::json)
      FROM weeks wk CROSS JOIN fleet
    )
  ) INTO result;
