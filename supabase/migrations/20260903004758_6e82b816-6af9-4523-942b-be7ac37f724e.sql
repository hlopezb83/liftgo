...
BEGIN
  ...
  -- MON-01: si el caller no manda lote, se resuelve el lote VIGENTE que
  -- contiene la factura. Un lote cancelado se borra (cancel_supplier_payment_batch),
  -- así que la existencia de la fila implica lote activo. Sin esto batch_id
  -- quedaba siempre NULL y un lote ya pagado podía cancelarse y reexportarse.
