...
BEGIN
  ...
  depreciation_per_month AS (
    SELECT
      rpm.month_key,
      SUM(f.acquisition_cost / 48.0) AS depreciation,
      jsonb_object_agg(f.name, f.acquisition_cost / 48.0)
  ...
  -- ============ COGS (NUEVO): basado en facturas de venta ============
  -- La fuente de COGS son las facturas de venta del período, NO el estatus
  -- del montacargas. Esto asegura que el COGS aparece en el mismo mes y
  -- cliente que el ingreso, evitando desfases por flips tardíos de status.
  ...
  -- Descuenta del costo de adquisición la depreciación ya reconocida en
  -- períodos previos (meses rentados antes de la fecha de facturación).
  months_rented_per_sale AS (
    SELECT
      sif.forklift_id,
      sif.event_date,
      COALESCE(SUM(
  ...
  cogs_per_sale AS (
    SELECT
      ...
      GREATEST(0,
        ...
        - COALESCE(sif.acquisition_cost, 0)
          * LEAST(48, COALESCE(mrp.months_rented, 0)) / 48.0
      ) AS book_value
    FROM sale_invoice_forklifts sif
    LEFT JOIN months_rented_per_sale mrp
      ON mrp.forklift_id = sif.forklift_id
     AND mrp.event_date = sif.event_date
  ...
  -- "Vendidos sin costo": equipos referenciados por facturas de venta del
  -- período cuyo acquisition_cost es nulo o cero (no se puede calcular COGS).
