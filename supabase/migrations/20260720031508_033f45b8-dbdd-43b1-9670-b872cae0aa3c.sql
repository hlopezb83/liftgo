...
-- PL-03: coverage window on supplier bills
ALTER TABLE public.supplier_bills
  ADD COLUMN IF NOT EXISTS coverage_start date,
  ADD COLUMN IF NOT EXISTS coverage_end date;

ALTER TABLE public.supplier_bills
  DROP CONSTRAINT IF EXISTS supplier_bills_coverage_range_chk;
ALTER TABLE public.supplier_bills
  ADD CONSTRAINT supplier_bills_coverage_range_chk
  CHECK (
...
-- PL-04: damage repair date + damage_charge invoice type
ALTER TABLE public.damage_records
  ADD COLUMN IF NOT EXISTS repaired_at timestamptz;

UPDATE public.damage_records
   SET repaired_at = updated_at
 WHERE repaired_at IS NULL AND status = 'repaired';

ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS invoice_type text NOT NULL DEFAULT 'standard';
ALTER TABLE public.invoices
  DROP CONSTRAINT IF EXISTS invoices_invoice_type_chk;
ALTER TABLE public.invoices
  ADD CONSTRAINT invoices_invoice_type_chk
  CHECK (invoice_type IN ('standard','damage_charge'));

-- Rewrite get_income_statement
...
BEGIN
  ...
  -- PL-04: use COALESCE(actual, estimated) and repaired_at fallback to created_at
  damage_by_month AS (
  ...
  -- PL-03: sb_lines with coverage-based proration in accrual mode.
  -- Bills without coverage keep the single-month behavior.
  sb_base AS (
    SELECT
  ...
  sb_accrual_no_coverage AS (
    SELECT
      ...
      sb.category, sb.description, sb.subtotal AS amount
    FROM sb_base sb
  ...
  sb_accrual_coverage AS (
    SELECT
      ...
      -- Prorate: (overlap_days_in_month / total_coverage_days) * subtotal
      (
        sb.subtotal
        * ( GREATEST(0,
