...
-- ============================================================
-- Paso 1 auditoría: vista única para "factura + saldo"
-- ============================================================
...
SELECT
  ...
  GREATEST(i.total - COALESCE(p.paid, 0), 0)::numeric       AS balance
...
COMMENT ON VIEW public.v_invoices_with_balance IS
  'Facturas con paid_amount y balance precalculados. Fuente única de verdad para cartera vencida, pronóstico de cobranza y aging. RLS aplica vía security_invoker.';
...
-- ============================================================
-- Refactor: get_financial_kpis usa la vista
-- ============================================================
...
BEGIN
  ...
  -- Cartera vencida actual = SUM(balance) de la vista
  ...
  WHERE v.status IN ('sent', 'partial', 'overdue')
    AND v.due_date < v_today;

  -- Cartera vencida histórica (pagos cortados a v_last_prev_month) — se mantiene cálculo inline porque depende de la fecha de pago.
...
-- ============================================================
-- Refactor: get_dashboard_stats usa la vista para outstanding_revenue y overdue_invoices
-- ============================================================
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'invoice_stats', (
      SELECT json_build_object(
        -- Saldo pendiente usando la vista unificada
        'outstanding_revenue', (
          SELECT COALESCE(SUM(v.balance), 0)
          FROM public.v_invoices_with_balance v
          WHERE v.status != 'paid' AND v.status != 'cancelled'
        ),
        'breakdown', COALESCE((
    ...
    -- Facturas vencidas usando la vista
    'overdue_invoices', (
