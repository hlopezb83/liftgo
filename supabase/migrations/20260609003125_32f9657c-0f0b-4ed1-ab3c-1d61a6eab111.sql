...
-- 1) Update income statement RPC to use supplier_bills
...
BEGIN
  ...
  FROM forklifts f
  WHERE (f.acquisition_cost IS NULL OR f.acquisition_cost <= 0)
    AND EXISTS (
...
-- 2) RLS policies for supplier-payment-receipts bucket
CREATE POLICY "Receipts read for admin/administrativo/auditor"
...
CREATE POLICY "Receipts insert for admin/administrativo"
...
CREATE POLICY "Receipts update for admin/administrativo"
...
CREATE POLICY "Receipts delete for admin/administrativo"
