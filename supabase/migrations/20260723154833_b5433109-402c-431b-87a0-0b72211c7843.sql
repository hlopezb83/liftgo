...
-- 1) Reescribir get_dashboard_stats con las 3 correcciones
  ...
  SELECT json_build_object(
    'fleet_counts', (
      SELECT json_build_object(
        ...
        -- BUG-FIX v7.200: "rented" = reservas confirmadas activas hoy (fuente única).
        -- Evita discrepancia entre tarjeta KPI y pie chart cuando forklifts.status
        -- diverge de la realidad de bookings.
        'rented', (
    ...
    'monthly_utilization', (
      ...
      -- BUG-FIX v7.200: (a) excluir 'sold' además de 'retired' (coincide con la tarjeta);
      -- (b) para el mes en curso, contar solo hasta CURRENT_DATE.
      bounded AS (
...
-- 2) Función de reconciliación del estatus de flota vs reservas activas
...
BEGIN
  ...
  -- 'available' -> 'rented' cuando hay reserva activa
  promote AS (
    UPDATE forklifts f
  ...
  -- 'rented' -> 'available' cuando ya no hay reserva activa
  demote AS (
    UPDATE forklifts f
