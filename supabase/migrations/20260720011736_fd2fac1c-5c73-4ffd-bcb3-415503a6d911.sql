...
BEGIN
  ...
  rental_by_customer AS (
    ...
    FROM (
      ...
      FROM inv_classified WHERE is_rental GROUP BY month_key, customer_name
  ...
  sales_by_customer AS (
    ...
    FROM (
      SELECT month_key, customer_name, SUM(subtotal) AS total
      FROM inv_classified WHERE NOT is_rental GROUP BY month_key, customer_name
    ) t GROUP BY month_key
  ),
  maint_by_month AS (
  ...
  -- BL-48: unificar supplier_bills + operating_expenses en un solo agregado
  -- por (month_key, category). Antes se ignoraba operating_expenses.
  expense_lines AS (
    -- Supplier bills (mismo cálculo que antes)
    SELECT
      CASE
    ...
    WHERE sb.status <> 'cancelled'
      ...
      END
    UNION ALL
    -- BL-48: Operating expenses (nómina, renta, servicios, etc.).
    -- expense_date es el único evento contable disponible; se usa igual para
    -- accrual y cash (no hay ciclo de pago propio).
    SELECT
  ...
  depreciation_per_month AS (
    SELECT rda.month_key,
      SUM((f.acquisition_cost / 48.0) * (rda.rented_days::numeric / rda.days_in_month)) AS depreciation,
      jsonb_object_agg(f.name, (f.acquisition_cost / 48.0) * (rda.rented_days::numeric / rda.days_in_month))
        FILTER (WHERE f.acquisition_cost IS NOT NULL AND f.acquisition_cost > 0 AND rda.rented_days > 0) AS depreciation_by_forklift
    FROM rented_days_agg rda
    JOIN forklifts f ON f.id = rda.forklift_id
    WHERE COALESCE(f.is_e2e, false) = false AND rda.rented_days > 0
  ...
  sale_invoice_forklifts AS (
    ...
    WHERE NOT ic.is_rental AND ic.quote_id IS NOT NULL AND COALESCE(f.is_e2e, false) = false
  ),
  rented_days_before_sale AS (
    SELECT sif.forklift_id, sif.event_date,
      COALESCE(SUM(GREATEST(0, LEAST(b.end_date, (sif.event_date - interval '1 day')::date) - b.start_date + 1)), 0) AS rented_days
    FROM sale_invoice_forklifts sif
    LEFT JOIN bookings b ON b.forklift_id = sif.forklift_id
      AND COALESCE(b.is_e2e, false) = false
      AND b.status IN ('confirmed','completed') AND b.start_date < sif.event_date
    GROUP BY sif.forklift_id, sif.event_date
  ),
  cogs_per_sale AS (
    SELECT sif.month_key, sif.customer_name, sif.forklift_name, sif.acquisition_cost,
      GREATEST(0, COALESCE(sif.acquisition_cost, 0)
        - COALESCE(sif.acquisition_cost, 0) * LEAST(1.0, COALESCE(rdbs.rented_days, 0)::numeric / (48 * 30.0))) AS book_value
    FROM sale_invoice_forklifts sif
    LEFT JOIN rented_days_before_sale rdbs
      ON rdbs.forklift_id = sif.forklift_id AND rdbs.event_date = sif.event_date
  ...
  WHERE COALESCE(f.is_e2e, false) = false
    ...
    AND EXISTS (
      ...
      WHERE b.forklift_id = f.id AND COALESCE(b.is_e2e, false) = false
        AND b.status IN ('confirmed','completed')
        AND b.start_date <= p_end_date AND b.end_date >= p_start_date
    );
