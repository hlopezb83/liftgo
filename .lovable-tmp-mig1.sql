-- R7 lote 1: correcciones de auditoría R7 (CxP, KPIs, ER, mantenimiento, vista de ocupación)
...
-- abandonado deja el lote vivo. Ahora libera también locks viejos con lote
-- vivo siempre que ese lote no tenga pagos registrados.
...
-- R7-12: fuente única de verdad para "qué locks son liberables" (la UI la usa
-- para habilitar el botón con el conteo real).
...
AS $fn$
  SELECT b.id
   ...
   WHERE b.payment_in_progress_at IS NOT NULL
     ...
     -- Lote "estancado": ningun lote vivo que contenga la factura tiene pagos
...
-- R7-06: idempotencia del cron de mantenimiento. El log generado por póliza
-- queda marcado con su póliza y mes; el índice único impide duplicarlo en un
-- reintento.
...
-- R7-18 + seguridad: los casts de fecha se anclan a Monterrey y la vista
-- vuelve a ejecutarse con los permisos de quien consulta.
...
BEGIN
  ...
  cn_base AS (
    ...
    WHERE cn.status NOT IN ('draft','cancelled')
      AND (COALESCE(pi.moneda, 'MXN') = 'MXN' OR COALESCE(NULLIF(pi.tipo_cambio, 0), 0) > 0)
  ...
  sb_accrual_no_coverage AS (
    SELECT
      ...
      sb.category, sb.description, sb.subtotal AS amount, sb.supplier_name, sb.id AS bill_id
    FROM sb_base sb
  ...
  sb_accrual_coverage AS (
    SELECT
      ...
      sb.supplier_name, sb.id AS bill_id
    FROM sb_base sb
  ...
  sb_cash AS (
    SELECT
      ...
      CASE WHEN COALESCE(sb.total, 0) > 0
        THEN sb.subtotal * ((sp.amount * sb.fx) / sb.total)
        ELSE sp.amount * sb.fx END AS amount,
      sb.supplier_name, sb.id AS bill_id
    FROM sb_base sb
  ...
  oe_dedup AS (
    ...
    WHERE d.date_key IS NULL
      AND (
        ...
        OR NOT EXISTS (SELECT 1 FROM sb_lines sl WHERE sl.bill_id = oe.supplier_bill_id)
      )
  ),
  expense_lines AS (
