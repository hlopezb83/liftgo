...
export function calculateRentalCost(
  ...
  if (mr > 0 && remaining >= 30) {
    ...
    items.push({ description: `Monthly rental`, quantity: months, unit_price: mr, total: months * mr });
    remaining -= months * 30;
  }
  ...
  if (wr > 0 && remaining >= 7) {
    ...
    items.push({ description: `Weekly rental`, quantity: weeks, unit_price: wr, total: weeks * wr });
    remaining -= weeks * 7;
  }

  // Remaining days
  if (remaining > 0 && dr > 0) {
    items.push({ description: `Daily rental`, quantity: remaining, unit_price: dr, total: remaining * dr });
  } else if (remaining > 0 && dr === 0) {
    // Fallback: if no daily rate, use weekly / 7 or monthly / 30
    const fallback = wr > 0 ? wr / 7 : mr > 0 ? mr / 30 : 0;
    if (fallback > 0) {
      items.push({ description: `Daily rental`, quantity: remaining, unit_price: Math.round(fallback * 100) / 100, total: Math.round(remaining * fallback * 100) / 100 });
    }
  }
