...
export default function SupplierDetailPage() {
  ...
  const expenseColumns: ColumnDef<LinkedExpense>[] = [
    ...
    { id: "amount", header: "Monto", accessorKey: "amount", meta: { kind: "money" }, cell: ({ row }) => <span className="font-mono">{formatCurrency(row.original.amount)}</span> },
  ];
  ...
  const maintenanceColumns: ColumnDef<LinkedMaintenance>[] = [
    ...
    { id: "cost", header: "Costo", accessorFn: (m) => m.cost ?? 0, meta: { kind: "money" }, cell: ({ row }) => <span className="font-mono">{formatCurrency(row.original.cost || 0)}</span> },
  ];
