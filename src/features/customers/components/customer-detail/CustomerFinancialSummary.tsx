    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <div className="flex justify-between text-sm">
          ...
          <span className="tabular-nums font-semibold">{formatCurrency(totalInvoiced)}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-muted-foreground">Total Pagado</span>
          <span className="tabular-nums font-semibold text-status-available">{formatCurrency(totalPaid)}</span>
        </div>
        <div className="flex justify-between text-sm border-t pt-2">
          <span className="font-medium">Saldo Pendiente</span>
          <span className={`tabular-nums font-bold ${outstanding > 0 ? "text-destructive" : ""}`}>{formatCurrency(outstanding)}</span>
        </div>
      </CardContent>
    </Card>
  );
