...
export function RevenueReport({ startDate, endDate }: Props) {
  ...
  const columns: ColumnDef<Row>[] = [
    ...
    { id: "invoiced", header: "Facturado", accessorKey: "invoiced", meta: { kind: "money" }, cell: ({ row }) => <span className="font-mono">{formatCurrency(row.original.invoiced)}</span> },
    { id: "paid", header: "Pagado", accessorKey: "paid", meta: { kind: "money" }, cell: ({ row }) => <span className="font-mono">{formatCurrency(row.original.paid)}</span> },
  ];
