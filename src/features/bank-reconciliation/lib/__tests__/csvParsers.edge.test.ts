...
describe("parseBankCsv — casos negativos y formatos alternos", () => {
  it("archivo completamente vacío devuelve error", async () => {
    const res = await parseBankCsv("", "generico");
  ...
  it("archivo con solo encabezado no produce líneas ni errores", async () => {
    const res = await parseBankCsv("Fecha,Desc,Monto,Ref", "generico");
    expect(res.lines).toHaveLength(0);
    expect(res.errors).toHaveLength(0);
  });

  it("monto cero se reporta como inválido", async () => {
...
05/05/2026,A,0,R1`;
  ...
  it("monto con paréntesis se interpreta como negativo", async () => {
    const csv = `Fecha,Desc,Monto,Ref
05/05/2026,A,(1234.56),R1`;
  ...
  it("acepta separador ';' y monto con símbolo $ y comas de miles", async () => {
    const csv = `Fecha;Desc;Monto;Ref
05/05/2026;DEPOSITO;"$1,234.56";R1`;
  ...
  it("fecha DD/MM/YY se expande a 20YY", async () => {
    const csv = `Fecha,Desc,Monto,Ref
05/05/26,A,100,R1`;
    const res = await parseBankCsv(csv, "generico");
  ...
  it("BBVA: tanto cargo como abono presentes → credit - |charge|", async () => {
    const csv = `Fecha,Desc,Cargo,Abono,Ref
05/05/2026,REVERSO,200,500,R1`;
    const res = await parseBankCsv(csv, "bbva");
    expect(res.lines).toHaveLength(1);
    expect(res.lines[0].signed_amount).toBe(300);
  });

  it("mezcla válidas e inválidas: reporta error pero conserva las válidas", async () => {
    const csv = `Fecha,Desc,Monto,Ref
...
07/05/2026,OK2,-50,R3`;
  ...
  it("ignora líneas en blanco intermedias", async () => {
    const csv = `Fecha,Desc,Monto,Ref
05/05/2026,A,100,R1
