...
export function PostInspectionInvoiceDialog({
  open, onOpenChange, booking, forklift, damageCost,
  ...
  const allItems: LineItem[] = [
    ...rentalItems,
    ...(damageCost > 0 ? [{ description: "Damage charges", quantity: 1, unit_price: damageCost, total: damageCost }] : []),
  ];
    ...
    createInvoice.mutate(
      {
        ...
        line_items: JSON.parse(JSON.stringify(allItems)),
        subtotal,
        tax_rate: taxRate,
      ...
      {
        onSuccess: (data) => {
          toast.success("Invoice generated from return inspection");
          onOpenChange(false);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Create an invoice for {booking.customer_name || "this rental"} based on the completed booking.
          </DialogDescription>
        </DialogHeader>
